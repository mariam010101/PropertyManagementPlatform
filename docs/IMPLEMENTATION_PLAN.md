...
[`docs/IMPLEMENTATION_GAP_ANALYSIS.md`](docs/IMPLEMENTATION_GAP_ANALYSIS.md) documents **26** gaps
(GAP-001..GAP-026) — some gaps span multiple tasks.
...
| --- | --- | --- | --- |
| Auth (BR-001, BR-008) | IMP-001, IMP-002, IMP-030, IMP-031, IMP-032 | 96% | suspended account state (IMP-032) blocked — non-MVP |
...
| Backend (MVP backend tasks IMP-001..006) | IMP-001..006 | 100% | — |
| Frontend (web) | IMP-007, IMP-026, IMP-041 | 91% | post-MVP pages (IMP-026) remain |
| Database/API | migrations + endpoint surface | 80% | MVP surface verified end-to-end; post-MVP endpoints unverified |
| Testing | IMP-006, IMP-040, IMP-041 | 100% | — |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| IMP-001 | Auth & identity: register/login/refresh/logout, password reset & change, email-confirmation gate, auth event audit, staff provisioning | Auth | P1 | — | FR-AUTH-001..008 | 0003, 0008 | COMPLETE | 100 | 10 |
| IMP-002 | RBAC: 5 roles, policies, admin user list/role assignment/deactivate/reactivate + admin UI | Auth | P1 | IMP-001 | FR-RBAC-001..005 | 0004 | COMPLETE | 100 | 8 |
| IMP-003 | Property: Property→Building→Unit CRUD, operational status, derived occupancy, manager scoping, search | Property | P1 | IMP-001 | FR-PROP-001..007 | 0006, 0010, 0011 | COMPLETE | 100 | 8 |
| IMP-004 | Resident: profile auto-provisioning, edit, effective-dated unit assignment, move-out, deactivate, occupancy history, search | Resident | P1 | IMP-001, IMP-003 | FR-RES-001..007 | 0008, 0010 | COMPLETE | 100 | 8 |
| IMP-005 | Maintenance: state machine, priorities, technician assignment, attachments, history, resident confirm + 7-day auto-close, persisted notifications | Maintenance | P1 | IMP-001, IMP-003, IMP-004 | FR-MNT-001..008 | 0009 | COMPLETE | 100 | 10 |
...
| IMP-007 | Frontend MVP pages + role-guarded routing (login, register, dashboard, properties, residents, maintenance, admin users) | Frontend | P1 | IMP-001..005 | FR-AUTH/RES/PROP/MNT sets | — | COMPLETE | 100 | 10 |

**Acceptance criteria**

- **IMP-001:** ✅ met (2026-09-16) — every FR-AUTH endpoint exists and enforces `RequireConfirmedEmail`; login is blocked for unconfirmed and deactivated users; refresh rotates and logout revokes the session; every auth event is persisted. Lockout, inactive-login and auth-event persistence are now directly verified by [`AuthApiTests.cs`](tests/PMP.Tests/Integration/AuthApiTests.cs:12) and [`AuthServiceTests.cs`](tests/PMP.Tests/AuthServiceTests.cs:17) (the previous lockout gap is closed).
- **IMP-002:** ✅ met (2026-09-16) — policies in [`AppPolicies`](src/PMP.Shared/Common/AppPolicies.cs) deny by default (whitelist `RequireRole`), admin endpoints are Administrator-only, role changes are audited (`RolesUpdated`/`AccountDeactivated` events) and the full least-privilege matrix for every MVP role is asserted by [`RbacMatrixApiTests.cs`](tests/PMP.Tests/Integration/RbacMatrixApiTests.cs:11); immediate role revocation (IMP-031) remains intact.
- **IMP-003:** ✅ met (2026-09-16) — occupancy is derived from active resident associations via [`UnitOccupancyProvider`](src/PMP.Modules.Resident/Abstractions/UnitOccupancyProvider.cs) (never stored), manager queries are scoped to assigned properties, duplicate unit numbers are rejected and now also API-tested ([`PropertyApiTests.cs`](tests/PMP.Tests/Integration/PropertyApiTests.cs:12)). Manager-role validation on create stays tracked as non-MVP hardening (GAP-007 / IMP-034).
- **IMP-004:** ✅ met (2026-09-16) — resident profile auto-created on registration (one profile per user); unit assignments are effective-dated and retained on transfer/move-out/deactivate; inactive units reject assignment; manager scoping denies cross-manager access; search filters by name/email (case-insensitive) and profile edit persists and is access-scoped. Verified by 12 unit tests in [`ResidentServiceTests.cs`](tests/PMP.Tests/ResidentServiceTests.cs:18) — the search and profile-edit paths were the last untested acceptance criteria — plus the MVP end-to-end API journey (IMP-006); `dotnet test PropertyManagement.sln` → **127 passed / 0 failed**.
- **IMP-005:** ✅ met (2026-09-16) — the transition map rejects invalid transitions, closed requests are immutable, status-change notifications are persisted, and the 7-day auto-close job is now directly verified by [`AutoCloseHostedServiceTests.cs`](tests/PMP.Tests/AutoCloseHostedServiceTests.cs:14) plus closed-immutability and notification assertions in [`MaintenanceServiceTests.cs`](tests/PMP.Tests/MaintenanceServiceTests.cs:141). Technician-role validation on assignment stays tracked as non-MVP hardening (GAP-007 / IMP-034).
...
- **IMP-007:** ✅ met (2026-09-16) — every MVP page (login, register, dashboard, properties, residents, maintenance, admin users) is reachable only under its role guard and renders real endpoint data; unauthenticated users are redirected to login and unauthorized users reach the forbidden state. Verified by the 12 route/page tests in [`AppRoutes.test.tsx`](frontend/src/test/AppRoutes.test.tsx:44) (`npm test` → 13 passed; `npm run build` green).
...
| IMP-041 | Frontend test infrastructure (Vitest/RTL) + core page/route tests | Testing | P2 | IMP-007 | — | COMPLETE | 100 | 4 |
...
- **IMP-041:** ✅ met (2026-09-16) — Vitest 5 + React Testing Library + jsdom + jest-dom installed and configured (`globals` mode in [`vite.config.ts`](frontend/vite.config.ts:8), setup file [`src/test/setup.ts`](frontend/src/test/setup.ts:1)); `npm test` runs **5 passed / 0 failed** — the existing smoke test ([`src/test/Icon.test.tsx`](frontend/src/test/Icon.test.tsx:1)) plus new MVP route-guard coverage in [`src/test/AppRoutes.test.tsx`](frontend/src/test/AppRoutes.test.tsx:1) proving (1) unauthenticated users are redirected from the protected `/properties` route to the login screen, (2) an authenticated `PropertyManager` reaches the manager-only properties route, (3) an unauthorized `Resident` is redirected to the forbidden screen, and (4) the representative Properties data page renders a property returned by the mocked API; `npm run build` → green.
...
| --- | --- | --- | --- | --- | --- |
| FR-AUTH-001..008 | 0003, 0008 | IMP-001, IMP-030 | [`AuthController.cs`](src/PMP.Api/Controllers/AuthController.cs:28) | [`AuthService.cs`](src/PMP.Modules.Auth/Services/AuthService.cs), [`TokenService.cs`](src/PMP.Modules.Auth/Services/TokenService.cs) | ✅ API integration (IMP-006) + 4 token unit tests (IMP-040): login/confirm gate/refresh/logout, token signature/expiry/subject/roles; ✅ own-profile update (IMP-030); ✅ lockout, inactive/unconfirmed login and auth-event persistence ([`AuthApiTests.cs`](tests/PMP.Tests/Integration/AuthApiTests.cs:12), [`AuthServiceTests.cs`](tests/PMP.Tests/AuthServiceTests.cs:17)) |
| FR-RBAC-001..006 | 0004 | IMP-002, IMP-031 | [`AuthController.cs`](src/PMP.Api/Controllers/AuthController.cs:127) | [`AppPolicies.cs`](src/PMP.Shared/Common/AppPolicies.cs), [`AppRoles.cs`](src/PMP.Shared/Common/AppRoles.cs:8) | ✅ API integration (IMP-006): anonymous 401 + per-role 403; ✅ role revocation on next request (IMP-031); ✅ full least-privilege matrix per role ([`RbacMatrixApiTests.cs`](tests/PMP.Tests/Integration/RbacMatrixApiTests.cs:11)) + role-change audit ([`AuthServiceTests.cs`](tests/PMP.Tests/AuthServiceTests.cs:17)) |
| FR-PROP-001..007 | 0006, 0010, 0011 | IMP-003, IMP-034 | [`PropertiesController.cs`](src/PMP.Api/Controllers/PropertiesController.cs:25) | [`PropertyService.cs`](src/PMP.Modules.Property/Services/PropertyService.cs) | ✅ API integration (IMP-006) + 2 unit tests ([`PropertyServiceTests.cs`](tests/PMP.Tests/PropertyServiceTests.cs:10)) + duplicate-unit API test ([`PropertyApiTests.cs`](tests/PMP.Tests/Integration/PropertyApiTests.cs:12)) |
...
| FR-MNT-001..008 | 0009 | IMP-005, IMP-034 | [`MaintenanceController.cs`](src/PMP.Api/Controllers/MaintenanceController.cs:27) | [`MaintenanceService.cs`](src/PMP.Modules.Maintenance/Services/MaintenanceService.cs) | ✅ API integration (IMP-006) + 5 unit tests ([`MaintenanceServiceTests.cs`](tests/PMP.Tests/MaintenanceServiceTests.cs:15)) incl. closed immutability + notification persistence; ✅ 7-day auto-close ([`AutoCloseHostedServiceTests.cs`](tests/PMP.Tests/AutoCloseHostedServiceTests.cs:14)) |
...
| 2026-09-15 | **Lease creation 500 fixed (`GAP-026` RESOLVED).** `POST /api/leases` threw `InvalidOperationException` → HTTP 500 on SQLite because the one-active-lease overlap check compared the string-persisted `LeaseStatus` enum and `DateTimeOffset` start/end dates inside one `AnyAsync`, which EF Core 9 cannot translate. The check now narrows to the unit in SQL and evaluates status + date overlap in memory, and `GetLeasesAsync`/`RunExpiryLifecycleAsync` apply their status filters in memory after materialization (same pattern as the existing SQLite `DateTimeOffset` ordering workaround). Added [`LeaseServiceSqliteTests`](tests/PMP.Tests/LeaseServiceSqliteTests.cs:1) (3 SQLite-backed tests). Suite 121 → **124 passed / 0 failed**; build 0 warnings / 0 errors; live smoke test over HTTP created a lease (`status=Active`) and an overlapping unit correctly returned 400 instead of 500. No task-weight change. |
| 2026-09-16 | **`IMP-041` COMPLETE — MVP frontend route-guard tests.** Vitest 5 + React Testing Library + jsdom + jest-dom installed; Vitest wired into [`vite.config.ts`](frontend/vite.config.ts:8) with `globals` mode and the [`src/test/setup.ts`](frontend/src/test/setup.ts:1) setup file. Core page/route tests added in [`src/test/AppRoutes.test.tsx`](frontend/src/test/AppRoutes.test.tsx:1): unauthenticated users are redirected from `/properties` to login, an authenticated `PropertyManager` reaches the allowed properties route, an unauthorized `Resident` is redirected to the forbidden screen, and the Properties data page renders a property from the mocked API. `npm test` → **5 passed / 0 failed**; `npm run build` → green. No production code changed. |
| 2026-09-16 | **`IMP-004` COMPLETE — resident search + profile-edit coverage.** The last uncovered acceptance paths — `GetResidentsAsync` search filtering (name/email) and `UpdateResidentAsync` profile edit (happy path + cross-manager denial) — are now unit-tested in [`ResidentServiceTests.cs`](tests/PMP.Tests/ResidentServiceTests.cs:18). No production code changed. `dotnet test PropertyManagement.sln` → **127 passed / 0 failed** (was 124). |
| 2026-09-16 | **MVP completion pass — `IMP-001`, `IMP-002`, `IMP-003`, `IMP-005`, `IMP-007` COMPLETE.** Closed the remaining MVP verification gaps without changing working behaviour: (1) `IMP-001` — lockout, inactive-login and auth-event persistence now verified by [`AuthApiTests.cs`](tests/PMP.Tests/Integration/AuthApiTests.cs:12) and [`AuthServiceTests.cs`](tests/PMP.Tests/AuthServiceTests.cs:17); (2) `IMP-002` — full least-privilege matrix per role in [`RbacMatrixApiTests.cs`](tests/PMP.Tests/Integration/RbacMatrixApiTests.cs:11) plus role-change audit in `AuthServiceTests`; (3) `IMP-003` — duplicate-unit rejection now API-tested in [`PropertyApiTests.cs`](tests/PMP.Tests/Integration/PropertyApiTests.cs:12) (manager-role validation on create remains non-MVP hardening, GAP-007/IMP-034); (4) `IMP-005` — 7-day auto-close verified in [`AutoCloseHostedServiceTests.cs`](tests/PMP.Tests/AutoCloseHostedServiceTests.cs:14) and closed-immutability + notification persistence in [`MaintenanceServiceTests.cs`](tests/PMP.Tests/MaintenanceServiceTests.cs:141) (technician-role validation remains non-MVP hardening, GAP-007/IMP-034); (5) `IMP-007` — all MVP pages/guards covered by the 12 route/page tests in [`AppRoutes.test.tsx`](frontend/src/test/AppRoutes.test.tsx:44). The only production change is the test seam on [`AutoCloseHostedService`](src/PMP.Modules.Maintenance/Background/AutoCloseHostedService.cs:47) (`CloseExpiredAsync` made internal + `InternalsVisibleTo`). `dotnet test PropertyManagement.sln` → **153 passed / 0 failed**; `npm test` → **13 passed / 0 failed**; `npm run build` green. MVP 89% → **100%**; overall 77% → **81%**. Confluence mirror sync pending. |
