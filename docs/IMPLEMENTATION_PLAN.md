...
| IMP-041 | Frontend test infrastructure (Vitest/RTL) + core page/route tests | Testing | P2 | IMP-007 | — | IN_PROGRESS | 25 | 4 |
...
- **IMP-041:** Infrastructure COMPLETE (2026-09-16) — Vitest 5 + React Testing Library + jsdom + jest-dom installed and configured (`globals` mode in [`vite.config.ts`](frontend/vite.config.ts:8), setup file [`src/test/setup.ts`](frontend/src/test/setup.ts:1)); `npm test` runs one real smoke test ([`src/test/Icon.test.tsx`](frontend/src/test/Icon.test.tsx:1)) that renders an existing component with jest-dom matchers → **1 passed**; `npm run build` still passes. **Remaining:** core page/route tests (login/route-guard + one data page) outstanding.
...
| 2026-09-16 | **`IMP-041` frontend test infrastructure IN_PROGRESS.** Vitest 5 + React Testing Library + jsdom + jest-dom installed; Vitest wired into [`vite.config.ts`](frontend/vite.config.ts:8) with `globals` mode and the [`src/test/setup.ts`](frontend/src/test/setup.ts:1) setup file; `npm test` script added; one smoke test ([`src/test/Icon.test.tsx`](frontend/src/test/Icon.test.tsx:1)) renders an existing component (jest-dom matchers) → **1 passed**; `npm run build` → green. **Remaining:** core page/route tests (login/route-guard + one data page) before IMP-041 can be marked COMPLETE. |
